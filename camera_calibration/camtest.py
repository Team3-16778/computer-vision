...
cap = cv2.VideoCapture('nvarguscamerasrc sensor-id={} ! video/x-raw(memory:NVMM), width=3280, height=2464, format=(string)NV12, framerate=(fraction)21/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink drop=1 sync=0 max-buffers=1'.format(sensor_id), cv2.CAP_GSTREAMER)
